...
import {
  LayoutDashboard, Users, UserCheck, LogOut, Menu, X,
  Flag, Briefcase, Archive, MessageSquare, ShieldCheck,
  CheckCircle2, WifiOff, Loader2, Bell, Search, AlertTriangle,
  Info, Check, HelpCircle,
} from 'lucide-react';
import { prefetchAll, adminApi } from '@/api/admin';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (pendingVerificationsCount > 0) {
        list.push({
          ...
          description: `${pendingVerificationsCount} identity verification request${pendingVerificationsCount > 1 ? 's' : ''} waiting for review.`,
      ...
      if (openReportsCount > 0) {
        list.push({
          ...
          description: `${openReportsCount} content report${openReportsCount > 1 ? 's' : ''} require attention.`,
      ...
      if (openTicketsCount > 0) {
        list.push({
          ...
          description: `${openTicketsCount} support inquiry ticket${openTicketsCount > 1 ? 's' : ''} awaiting response.`,
  ...
  const bannerMap: Record<PrefetchStatus, { bg: string; icon: React.ReactNode; text: string } | null> = {
    idle:    null,
    loading: { bg: 'bg-blue-500/90',    icon: <Loader2 className="w-4 h-4 animate-spin" />,  text: 'Loading all sections in background…' },
    ready:   { bg: 'bg-emerald-500/90', icon: <CheckCircle2 className="w-4 h-4" />,           text: 'All sections loaded — navigation is instant.' },
    error:   { bg: 'bg-red-500/90',     icon: <WifiOff className="w-4 h-4" />,                text: 'Could not reach server. Check your connection.' },
  };
  ...
  const NotificationButton = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className="relative">
      <button
        ...
        className="p-2.5 bg-white/50 hover:bg-white text-ink hover:text-primary rounded-2xl border border-ink-faint/30 hover:border-primary/20 shadow-sm transition-all relative"
      ...
      {dropdownOpen && (
        <>
          ...
          <div className={`
            absolute mt-3 w-[22rem] bg-white/95 backdrop-blur-xl border border-ink-faint/40
            rounded-3xl shadow-xl z-40 overflow-hidden animate-fade-in divide-y divide-ink-faint/20
            ${isMobile ? 'fixed top-18 left-4 right-4 mx-auto w-auto' : 'right-0 top-full'}
          `}>
            {/* Header */}
            <div className="px-5 py-4 flex justify-between items-center bg-paper-cream/30">
              <div className="flex items-center gap-2">
                <h3 className="font-display font-bold text-ink">Attention Required</h3>
                {unreadCount > 0 && (
                  <span className="text-[10px] font-body font-bold bg-status-error/15 text-status-error px-2 py-0.5 rounded-full uppercase tracking-wider">
                    {unreadCount} New
                  </span>
                )}
              </div>
              {unreadCount > 0 && (
                <button
                  ...
                  className="text-xs font-body font-semibold text-primary hover:text-primary-dark transition-colors flex items-center gap-1"
                >
            ...
            {/* Notifications List */}
            <div className="max-h-80 overflow-y-auto divide-y divide-ink-faint/20">
              {notifications.length === 0 ? (
                <div className="px-5 py-8 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm font-body font-semibold text-ink">All Clear!</p>
                  <p className="text-xs font-body text-ink-muted mt-1">No items require administrative attention.</p>
                </div>
              ) : (
                notifications.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        w-full text-left px-5 py-4 transition-all flex gap-3 hover:bg-paper/50
                        ${isRead ? 'opacity-60 bg-white/40' : 'bg-white'}
                      `}
                    >
                      {/* Priority Graphic (Shape + Icon) */}
                      <div className={`
                        w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center shadow-inner
                        ${isAttention
                      ...
                      `}>
                        {isAttention ? (
                          <AlertTriangle className="w-4 h-4" aria-label="Requires Attention" />
                        ) : (
                          <Info className="w-4 h-4" aria-label="Important Update" />
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1.5">
                          <span className={`text-xs font-body font-bold uppercase tracking-wider ${isAttention ? 'text-status-error' : 'text-blue-500'}`}>
                            {isAttention ? 'Requires Attention' : 'Important Update'}
                          </span>
                          {!isRead && (
                            <span className="w-2 h-2 rounded-full bg-primary flex-shrink-0" />
                          )}
                        </div>
                        <h4 className="text-sm font-body font-bold text-ink mt-0.5 truncate">
                          {item.title}
                        </h4>
            ...
            {/* Footer */}
            <div className="px-5 py-3.5 bg-paper/30 text-center">
  ...
  return (
    <div className="min-h-screen bg-paper flex relative overflow-hidden">
      ...
      `}>
        <div className="h-20 items-center px-8 border-b border-ink-faint/30 lg:flex hidden">
      ...
      <div className="flex-1 flex flex-col min-h-screen relative overflow-x-hidden">
        ...
        <header className="hidden lg:flex h-20 bg-white/45 backdrop-blur-xl border-b border-ink-faint/30 items-center justify-between px-10 sticky top-0 z-20">
          {/* Breadcrumbs */}
          <div className="flex items-center gap-2">
            <span className="text-xs font-body font-bold uppercase tracking-wider text-ink-muted">Dashboard</span>
        ...
        {/* Main Content Area */}
        <main className="flex-1 max-w-[1400px] w-full mx-auto z-10 relative">
