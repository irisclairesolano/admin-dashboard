        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left font-body">
            <thead className="bg-white/50 border-b border-ink-faint/50">
              <tr>
                <th className="px-8 py-5 font-body font-semibold text-ink-soft text-sm uppercase tracking-wider">Job Details</th>
                <th className="px-8 py-5 font-body font-semibold text-ink-soft text-sm uppercase tracking-wider">Employer</th>
                <th className="px-8 py-5 font-body font-semibold text-ink-soft text-sm uppercase tracking-wider">Compensation & Slots</th>
                <th className="px-8 py-5 font-body font-semibold text-ink-soft text-sm uppercase tracking-wider">Status</th>
                ...
                filteredJobs.map((job) => (
                  <tr key={job.id} className="hover:bg-white/60 transition-colors duration-200">
                    <td className="px-8 py-5">
                      <div className="font-body font-bold text-ink text-lg">{job.title}</div>
                      <div className="flex items-center text-xs text-ink-muted mt-2">
                        <span className="px-2.5 py-1 rounded-md bg-accent-sky text-primary-dark font-body font-semibold shadow-sm mr-3 border border-white/50">{job.category}</span>
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center">
                        <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-accent-peach to-accent-peachBright/50 flex items-center justify-center text-primary-dark font-body font-bold text-sm shadow-inner mr-3">
                          {(job.employer?.name || 'U').charAt(0)}
                        </div>
                        <div>
                          <div className="font-body font-bold text-ink">{job.employer?.name || 'Unknown'}</div>
                          <div className="text-xs text-ink-soft mt-0.5">{job.employer?.email}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="text-sm font-numeric font-bold text-ink flex items-center">
                      ...
                      <div className="text-xs text-ink-soft mt-1.5 font-body font-semibold bg-paper px-2.5 py-1 rounded border border-ink-faint/50 inline-block">Slots: {job.slots}</div>
                    </td>
                    <td className="px-8 py-5">
                      <span className={`px-4 py-1.5 rounded-full text-xs font-body font-bold uppercase tracking-wide shadow-sm ${
